...
{
    uint32_t n_item;
    n_item = POWER_OF_2(maxLookupLen);
    
    lkt_t *lkt;
    lkt = (lkt_t *)malloc(sizeof(lkt_t)+(n_item+3)*sizeof(uint32_t));
    lkt->maxLookupLen = maxLookupLen;
    lkt->n_item = n_item;
    lkt->item = (uint32_t *)(lkt+1); 
    memset(lkt->item, 0, (lkt->n_item+3)*sizeof(uint32_t));
    return lkt;
}
...
lkt_t *lkt_restore(const char *fn_lkt)
{
    ...
    fread(lkt->item, sizeof(uint32_t), lkt->n_item+3, fp_lkt);
    fclose(fp_lkt);
    return lkt;
}
void lkt_build_lookuptable(const char *fn_pac, lkt_t *lkt)
{
    ...
    if(fp_pac == NULL){
        fprintf(stderr, "[build_lookuptable]:read pac file %s open fail!\n", fn_pac);
        exit(1);
    }
    ...
    {
        //if(i%1000000 == 0) fprintf(stderr,"iter :%u\n", i );
        for(j = 0; j < LookUpSeqLen; ++j)
        {
    ...
    //accumulate number
    fprintf(stderr, "n_item = %u\n", lkt->n_item);
    for(i = 1; i < lkt->n_item+3; ++i)
    {
        item[i] += item[i-1];
    }
...
void lkt_dump(const char *fn_lkt, lkt_t *lkt)
{
    ...
    //fwrite(lkt, 1, sizeof(lkt_t)+lkt->n_item * sizeof(uint32_t), fp_lkt);
    fwrite(&lkt->maxLookupLen, sizeof(int), 1,fp_lkt);
    fwrite(lkt->item, sizeof(uint32_t), lkt->n_item+3, fp_lkt);
    fclose(fp_lkt);
...
void build_lookuptable_backward(const char* fn_lkt_forward, const char *fn_lkt_backward)
{
    ...
    fwrite(lkt_back, 1, sizeof(lkt_t)+lkt_back->n_item * sizeof(uint32_t), fp_lkt_back);
    lkt_destroy(lkt);
    fclose(fp_lkt_back);
