...
int main(int argc, char *argv[]){
	...
	uint8_t  *mod_idx; 
	struct SeedIdx *sIdx;
	if(NULL == (sIdx = (struct SeedIdx*)malloc(sizeof(struct SeedIdx)))){
	    perror("error...[ malloc(sizeof(struct SeedIdx)) ]");
	    exit(1);
	}
	...
	//数组初始化----------------------------------------------------
	FILE *fp;
	if((fp=fopen64(f->seedidx[0],"rb"))==NULL){
	    fprintf(stderr, "can't open file:%s\n", f->seedidx[0]);
	    exit(1);
	}

	//uint32_t numread=fread(cur_seedidx,3*sizeof(uint32_t),LenExtIdx[0],fp);
	uint32_t numread=fread(cur_seedidx,2*sizeof(uint32_t),LenExtIdx[0],fp);
...
//fp_extidx = fpw[5];
        ...
        /*  
        if(repNum == 0) {
            for(idx = out_buf[1]; idx > out_buf[1] - 256; --idx) {
                fprintf(stderr, "jmp_idx[%d] = %d\n", idx, jmp_idx[idx]); 
            }
            exit(1);
        }
        */
...
printf("%u, len_ext_idx = %u\n", __LINE__, len_ext_idx);
        /*  
...
int cut_ext_seq(uint8_t *pac, bwt_t *bwt, bwtint_t k, bwtint_t l, int offset, uint32_t (*seq_buf)[2])
{
...
void getBlckData(uint32_t sidx[2],uint32_t num_ext, struct SubBuf *sub, idx_t *idx){
    ...
    int n_r = cut_ext_seq(pac, bwt, k, l,  off_pos, sub->seqR_buf);//right seq buf
    //log_seqbuf(n, sub->seqR_buf);
    off_pos = -16; 
    int n_l = cut_ext_seq(pac, bwt, k, l,  off_pos, sub->seqL_buf);//left seq buf
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//下面代码段完成以下几个功能：
