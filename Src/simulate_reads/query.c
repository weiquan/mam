...
int query_read_seq(queryio_t *qs, query_t *query)
{
    ...
    if(kseq->name.s != NULL) {
        ...
        memcpy(query->name, kseq->name.s, kseq->name.l);
    }      
    if(kseq->comment.s != NULL) {
        //query->comment = strdup(kseq->comment.s);
        memcpy(query->comment, kseq->comment.s, kseq->comment.l);
    }
    //read seq and generate reverse seq

/*  
    query->seq = (uint8_t *)calloc(l_seq, 1);
    query->apos = (uint8_t *)calloc(l_seq, 1);
...
*/
    ...
    for(i = 0; i < l_seq; ++i){
        ...
        if(nt>3) { 
            query->apos[query->n_ambiguous] = i;
            ++query->n_ambiguous;
            query->seq[i] = 0;
            query->rseq[l_seq-i-1] = 3; 
        } else{
            query->seq[i] = nt; 
            query->rseq[l_seq-i-1] = 3-nt; 
        } 
    }
    ...
    if(kseq->qual.l > 0){
        ...
        */
        ...
        memcpy(query->qual, kseq->qual.s, kseq->qual.l);
