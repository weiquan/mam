...
void sam_add_xa(kstring_t *s, idx_t *index, query_t *query, int is_cigar);
...
void aln_samse(idx_t *index, query_t *query, const aln_opt_t *opt)
{
    ...
    if(query->seq_end < query->l_seq) ksprintf(query->cigar, "%uS",query->l_seq - query->seq_end);
        ...
        switch(op){
            case 'M':
                ...
                for(i = 0; i < n; ++i){
                    ...
                    assert(ref_pos < index->bwt->seq_len);
...
#define STRAND_REV 1
void alnpe_sam(idx_t *index, query_t *q, const aln_opt_t *opt)
{
    bntseq_t *bntseq = index->bns;
    int i, j;
